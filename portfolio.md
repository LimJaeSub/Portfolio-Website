...
### 2026-09-17 · 전역 컨트롤을 화면 전용 위젯 안에 넣었다

무엇이 안 됐는지: 테마 토글을 SideNav 리모콘 하단에 넣었는데(결정 38),
목록·상세 페이지에서는 토글할 방법이 없었다. SideNav는 홈에만 렌더된다.

원인: 테마는 전 페이지에 적용되는 전역 상태인데, 컨트롤을 홈 전용 위젯 안에 넣었다.
결정 38을 내릴 때는 상세 페이지가 아직 없었다.

대안 검토: SideNav를 전 페이지에 넣는 안은 폐기했다. 섹션 이동이 `getElementById`로
동작해서, 섹션이 없는 화면에서는 버튼 4개가 조용히 아무 일도 하지 않는다.
활성 표시도 `mypage`에 붙박이로 남는다.

해결: 토글을 SideNav에서 분리해 `App.jsx`의 `<Routes>` 밖에 두고
우측 하단에 `z-50`으로 독립 고정 (결정 41).

배운 점: 전역 컨트롤을 특정 화면 전용 위젯 안에 넣으면 그 위젯의 수명에 묶인다.
"둘 곳이 거기뿐"이라고 판단하기 전에, 고정 위치를 새로 잡는 선택지를 먼저 확인할 것.
